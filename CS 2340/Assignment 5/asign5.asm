...
#NET-ID: AVD210006
#Purpose: Write a program to take floating point input from the user then sort the data in ascending order,
#after sorting, print the numbers in order, Then find and print the count, sum, and average if the numbers.
#Learn about Coprocessecer one and how assembly language sees floating point completely different than Integer values 

#functions:
#1.sort: has one parameter which is the size that will be passed in $a0, will sort Double precision floating point  numbers in memory 
#the label used to sort the numbers will be under nums.

#2.print: has one parameter which is the size that will be passed in $a0, will print all the double precision floating point numbers in memory under nums label 
#while printing the numbers it will add each number to a sum, after printing the numbers it will print the count, sum, calculate the average by dividing sum/count 
...
prompt:	.asciiz	"Enter number: "	#prompt for the user to enter a number
sorted:	.asciiz	"Sorted list:\n"	#will show the sorted numbers after sort 
cnt:	.asciiz	"Count: "		#will show how many numbers were entered 
sum:	.asciiz	"Sum: "			#will show the total of all the numbers summoned up
Avrg:	.asciiz	"Average: "		#will show the average of all the numbers 
...
newLine:.asciiz	"\n"			#will be used as a newline character 
	...
	.text 
#main
main:	la	$a1,nums		#load the address where we are storing the float values at  
...
ask:	li	$v0,SysPrintString 	#load the print string function 
	...
	addi 	$a1,$a1,8		#add 8 to the address as a double is 8 bytes long 
	addi 	$t0,$t0,1		#add 1 to the counter of the current numbers 
	j 	ask 			# to go back and ask for another number 
exit:	move	$a0,$t0 		#move the count of numbers into $a0
	...
	beq	$a0,1,noSort		#if there was only 1 number input then we do not need to sort it so jump around the sort function 
	jal	sort			# calls the sort function which will sort the "array" in the address of nums
...
#printing function 
print:	la	$a1,nums 		#load the address of the numbers into $a1
	move	$t0, $a0		#move the counter from $a0 to $t0 
	move	$a2, $a0		#move the cou
	mtc1	$zero,$f4		#move zero into $f4 
	cvt.d.w	$f4,$f4			#convert the zero into a double, register $f4 is the sum accumulator 
	li	$v0,SysPrintString 	#load the print string function
	la	$a0,sorted		#load the "sorted list" string 
	syscall				#print the string 
Ploop:	beqz 	$t0,pInfo 		#if $t0 is equal to 0 which is the count inside the array start printing the info, Ploop means print loop
	...
	add.d	$f4,$f4,$f12 		#add double number to accumulator 
	subi	$t0,$t0,1		#subtract one from the size 
	addi	$a1,$a1,8		#add 8 to the address which will move it one number forward 
...
pInfo:	li	$v0,SysPrintString 	#load the print string function
	...
	mov.d	$f12,$f4 		#move the value from the accumulator into $f12 
	li	$v0,SysPrintDouble 	#load the print Double function
	syscall				#print the sum from the accumulator
	li	$v0,SysPrintString 	#load the print String function
	la	$a0,newLine		#load the new line 
	syscall				#print the new line 
	la	$a0,Avrg		#load the "Average" String 
	syscall 			#print the "Average" String
	li	$v0,SysPrintDouble 	#load the print Double function 
	...
	syscall				#print the double
	jr		$ra		#jump back to call address 

#sorting function 	
sort:	la	$a1, nums		#load the address of the "array in to $a1"
	li	$t0, 0 			#$t0 will act as bool of swapping so will continue to loop while swapping 
	move	$t1,$a0			#move the count of numbers which is stored in $a0 into $t1 
	subi	$t1,$t1,1		#subtarct one as the number of comparisons we have to make is n-1 
for:	lwc1	$f0,($a1)		#load first part of the num into register $f0 
	...
	bc1f	noSwap			#if false then we do not swap them fall through 
...
noSwap:	addi	$a1,$a1,8		#add 8 bytes to the the address in $a1 which will move one number forward 
	subi	$t1,$t1,1		#subtract 1 from $t1 which has the number of comparisons needed to be made
	bnez	$t1,for			#contunie branching until the number of comparisons reaches 0 
	bnez	$t0,sort		#contnie sorting until $t0 is equal to 0 which means no swaps where made
	jr	$ra			#jump back to the call address 
