...
#and calculate the all prime numbers up to that number using sieve algoritm
#and display them 
#-------------------------------------------------------------------------------------------------------#
...
input:	li		$v0,SysPrintString	#load print String function
		...
		bgt		$v0,MAXNUM,error	#check if number is above max if so throw an error 
		blt		$v0,MINNUM,error	#check if number is below min if so throw an error
...
calc:	andi	$t1,$v0,7			#check if theres a remainder if so $t1 is 1 then theres a remainder 
		srl		$t0,$v0,3			#divide by 8 to get number of bytes needed
		move 	$t2,$v0				#store the input value in $t2 
		beqz	$t1,noA				#if there is no remainder branch as we dont need to add another byte
...
noA:	li		$v0,SysAlloc			#load the memory allocation function
		...
		syscall 						#allocate needed amount of memory 
		move	$a1,$v0				#store the begining adress in $a1 
		#intilizing the bits to all high values
		la		$t3,($t2)				#set $t3 to the input number so we now how many numbers to loop through inside the memory
		addi	$t3,$t3,1				#add one to number of times to loop through as we 
...
setArr:	sb		$t4,($v0)			#intilize the array to all 0xFF,store $t4 which is 0xFF into the bit at current memory location
		...
		bnez	$t3,setArr			#if the input number is not zero contunie to set array to all 0xFF
		#calculating prime numbers
		li		$t4,0				#itilize a number that will be used to flip the bits off
		li		$t5,2				#keeps track of currecnt index 
		move	$t1,$t2				#move the user input from $t2 to $t1 will keep track number of times needed to loop
		srl		$t1,$t1,1				#divide the number of items by 2  sisnse the algoritm termintes at n/2
prime:	bgt		$t5,$t1,exitL			#if the number in $t5 which is the current index is greater than $t1 which is the total times we want to loop we want to exit 
		move 	$a0,$a1				#reset pointer that points to the beging 
		...
		bnez	$t7,loop				#if number is not zero we want calculate each number that is multiple of that numebr
		addi	$t5,$t5,1				#iincrement the current value 
		j		prime 				#go back and check next number 
loop:	add		$a0,$a0,$t5			#Prime increment by the multiple of it which is stored in $t5
		add		$t6,$t6,$t5			#add to $t6 of current index to know when to break out of loop
		sb		$t4,($a0)			#set byte at location equal to zero 
		ble		$t6,$t2,loop			#if  the value if $t6 is less than $t2 contunie looping $t6 stores current index position and $t2 stores total input
		addi	$t5,$t5,1				#after loop is done add one to the current index value 
		j 		prime				#contunie looping 
		#after loop is done
exitL:	addi	$a1,$a1,2			#Exit loop,add 2 to stored adress will go to the number 2  
...
Ploop:	lb		$t3,($a1)			#Print Loop, load the current byte into $t3 
		addi	$a1,$a1,1 			#increment the pointer one forward so its pointing to the next number 
		...
		bnez 	$t1,Ploop			#if value in $t1 is not zero then we are not at 10 numbers on a line yet jump back  
