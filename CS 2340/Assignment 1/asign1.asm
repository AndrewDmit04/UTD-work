#Program written by Andrew Dmitrievsky
			...
			.include	"Syscall.asm"							#include equvilant
			.data											
...
Read:	li 		$v0, SysPrintString		 #load register with printing String function
			...
			li		$v0,	SysReadInt			 #read in an integer, and create loop for entering numbers
			syscall
			add		$a1,$a1,$v0						 #add the integer to the current sum
			addi	$a2,$a2,1							 #add 1 to the current count 
			bnez  	$v0,Read						 #if the number entered by the user isnt zero go back to the Read branch
			...
			la		$a0,lenNum						 #load the assosiated register with the string we want to print
			...
			la		$a0,($a2)							 #load the assosiated register with the int we want to print
			syscall											 #call the function and print the string
			#printing a endline 
			li 		$v0,SysPrintString		 #load the  print String function
			la		$a0,endLine						 #load the assosiated register with the string we want to print
			syscall											 #call the function and print the string
			#exiting program
			li		$v0,SysExit
			syscall
